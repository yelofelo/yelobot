async def save_roles(member, mongo_db):
    ...
    await collection.delete_many({'user_id': member.id, 'server_id': member.guild.id})
    await collection.insert_one({'user_id': member.id, 'server_id': member.guild.id, 'roles': roles})

async def assign_roles(member, mongo_db):
    collection = mongo_db['LeaverRoles']

    doc = await collection.find_one({'user_id': member.id, 'server_id': member.guild.id})
    if doc:
...
async def assign_roles_on_startup_impl(bot, mongo_db):
    ...
    for server_id in servers_to_delete:
        await collection.delete_many({'server_id': server_id})
